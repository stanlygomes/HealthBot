...
def predictDisease(selected_symptoms):
    ...
    # Format entered symptoms into dataframe for model prediction
    df_predict = pd.DataFrame(columns=list(features.keys()))
    df_predict.loc[0] = np.array(list(features.values()))
    
    model = load("./Dataset_Tests/Dataset_1/models/DecisionTree.joblib")
...
iface = gr.Interface(
    ...
    "text",
    description="Provide the symptoms you are exhibiting currently and press the submit button to recieve a predicted illness you may have. DISCLAIMER: \
    This program is trained on a very overfitted model with not enough data, so the prediction may be inaccurate."
)
